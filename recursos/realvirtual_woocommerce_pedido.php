<?php
	class RealVirtualWooCommercePedido
	{
		...
		static function obtenerPedidosFacturacion($fechaDesde, $fechaHasta, $tipoFecha, $estadoOrden, $precision_decimal, $rfcEmisor, $usuarioEmisor, $claveEmisor, $urlSistemaAsociado, $idioma)
		{
			if(!isset($precision_decimal))
				$precision_decimal = '2';
			...
			try
			{
				...
				$dateDesde = new DateTime($fechaDesde.' 00:00:00');
				$dateHasta = new DateTime($fechaHasta.' 23:59:59');
				
				//$dateDesde->setTimezone(new DateTimeZone('UTC'));
				//$dateHasta->setTimezone(new DateTimeZone('UTC'));
				
				$fechaDesdeFormateada = $dateDesde->getTimestamp();
				$fechaHastaFormateada = $dateHasta->getTimestamp();
				
				$parametros = array(
					'limit'	=> -1,
					'date_created'=> $fechaDesdeFormateada.'...'.$fechaHastaFormateada,
					'return' => 'ids',
					'orderby' => 'date',
					'order' => 'DESC'
				);
				
				if($tipoFecha == 'created')
				{
					$parametros = array(
						'limit'	=> -1,
						'date_created'=> $fechaDesdeFormateada.'...'.$fechaHastaFormateada,
				...
				else if($tipoFecha == 'modified')
				{
					$parametros = array(
						'limit'	=> -1,
						'date_modified'=> $fechaDesdeFormateada.'...'.$fechaHastaFormateada,
						'return' => 'ids',
						'orderby' => 'date',
						'order' => 'DESC'
					);
				}
				else if($tipoFecha == 'paid')
				{
					$parametros = array(
						'limit'	=> -1,
						'date_paid'=> $fechaDesdeFormateada.'...'.$fechaHastaFormateada,
						'return' => 'ids',
						'orderby' => 'date',
						'order' => 'DESC'
					);
				}
				else if($tipoFecha == 'completed')
				{
					$parametros = array(
						'limit'	=> -1,
						'date_completed'=> $fechaDesdeFormateada.'...'.$fechaHastaFormateada,
						'return' => 'ids',
						'orderby' => 'date',
						'order' => 'DESC'
					);
			    ...
			    foreach ($pedidosFinales as $pedidoFinal)
				{
					...
					$fechaPedido = $pedido->get_date_created();
					...
					{
						$fechaPedido = $pedido->get_date_modified();
					}
					else if($tipoFecha == 'paid')
					{
						$fechaPedido = $pedido->get_date_paid();
					}
					else if($tipoFecha == 'completed')
					{
						$fechaPedido = $pedido->get_date_completed();
					}
					
					$fechaPedido->setTimezone(new DateTimeZone($timezone_string));
				...
				(
					'success' => true,
					'pedidosHTML' => $pedidosHTML,
					'pedidosJSON' => $dateDesde->format('d/m/Y H:i:s').'...'.$dateHasta->format('d/m/Y H:i:s'),
					'pedidos' => $pedidos
				);
